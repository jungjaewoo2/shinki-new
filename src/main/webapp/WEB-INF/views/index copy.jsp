        ...
        <div class="container-fluid">
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between gap-3 gap-lg-2">
                <div class="type-box p-4 p-lg-5 rounded-5 cursor-pointer" data-bs-toggle="modal" data-bs-target="#type-1">
                    <div class="d-flex gap-lg-5 align-items-center flex-column flex-lg-row justify-content-center">
                        <div># CCU 최대 10대</div>
                        <div># 고가용성(HA)</div>
                        <div># 중앙집중형 관리</div>
                    </div>
...
<!--<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const typeSection = document.querySelector('#type');
        ...
        // 모든 .pill 요소 및 #application의 h2 요소 선택
        const sections = [businessSection, typeSection, useCasesSection, applicationSection];
        const pills = sections.reduce((acc, section) => section ? [...acc, ...section.querySelectorAll('h2 > span.pill')] : acc, []);
        const applicationH2s = applicationSection ? applicationSection.querySelectorAll('h2') : [];

        // .pill 및 #application h2에 트랜지션 적용
        pills.forEach(pill => {
            pill.style.transition = 'border 0.1s ease, color 0.1s ease';
            pill.style.boxSizing = 'border-box';
        });
        applicationH2s.forEach(h2 => {
            h2.style.transition = 'color 0.1s ease';
        });
        ...
        const updateStyles = () => {
            ...
            // #business가 뷰포트에 들어오자마자
            if (businessRect.top < viewportHeight) {
                document.body.style.backgroundColor = '#fff';
                document.body.style.color = '#333';
                if (prevButton) prevButton.style.color = '#333';
                if (nextButton) nextButton.style.color = '#333';
                pills.forEach(pill => {
                    pill.style.setProperty('border', '1px solid rgba(0, 0, 0, 0.15)', 'important');
                    pill.style.setProperty('color', '#333', 'important');
                });
                applicationH2s.forEach(h2 => {
                    h2.style.setProperty('color', '#333', 'important');
                });
            }
            // #application의 하단이 뷰포트 상단에 도달
            else if (applicationRect.bottom <= 0) {
                document.body.style.backgroundColor = '#070b1a';
                document.body.style.color = '';
                if (prevButton) prevButton.style.color = '';
                if (nextButton) nextButton.style.color = '';
                pills.forEach(pill => {
                    pill.style.setProperty('border', '1px solid rgba(255, 255, 255, 0.15)', 'important');
                    pill.style.setProperty('color', 'rgba(255, 255, 255, 0.8)', 'important');
                });
                applicationH2s.forEach(h2 => {
                    h2.style.setProperty('color', '', 'important');
                });
            }
            // 그 외
            else {
