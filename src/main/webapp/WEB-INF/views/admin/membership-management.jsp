...
<jsp:include page="header.jsp" />
 ...
 <div id="" class="page d-flex flex-column h-100">
     <div class="align-items-center d-flex flex-column flex-lg-row justify-content-between page-header">
         <div class="page-title">가입회원 관리목록</div>
         <div class="breadcrumb">관리자페이지 > 회원관리 > 가입회원 관리</div>
     </div>
     ...
     <div class="card d-flex flex-column gap-0 gap-lg-3 flex-fill overflow-auto">
        <div class="card d-flex align-items-center flex-lg-row flex-column justify-content-end gap-2">
            <form method="get" action="/admin/membership-management" class="d-flex gap-2">
         ...
         <nav aria-label="Page navigation" class="mt-3">
             <ul class="pagination justify-content-center">
                <c:if test="${currentPage > 0}">
                    <li class="page-item">
                        <a class="page-link" href="/admin/membership-management?page=${currentPage - 1}&size=${size}<c:if test='${not empty search}'>&search=${search}</c:if>">이전</a>
                    </li>
                </c:if>
                 <c:forEach begin="0" end="${totalPages - 1}" var="i">
                     <c:choose>
                        ...
                        <c:otherwise>
                            <li class="page-item">
                                <a class="page-link" href="/admin/membership-management?page=${i}&size=${size}<c:if test='${not empty search}'>&search=${search}</c:if>">${i + 1}</a>
                            </li>
                        </c:otherwise>
                     </c:choose>
                 </c:forEach>
                <c:if test="${currentPage < totalPages - 1}">
                    <li class="page-item">
                        <a class="page-link" href="/admin/membership-management?page=${currentPage + 1}&size=${size}<c:if test='${not empty search}'>&search=${search}</c:if>">다음</a>
                    </li>
                </c:if>
             </ul>
         </nav>
         ...
         <!-- 폼과 버튼들 -->
         <form id="withdrawForm" action="/admin/membership-management/withdraw" method="post" style="display: none;">
             <input type="hidden" name="selectedMembers" id="selectedMembersInput">
         </form>
         
         <form id="deleteForm" action="/admin/membership-management/delete" method="post" style="display: none;">
...
function withdrawSelectedMembers() {
    ...
    if (confirm('선택된 회원을 탈퇴 처리하시겠습니까?')) {
...
function deleteSelectedMembers() {
    ...
    if (confirm('선택된 회원을 완전히 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.')) {
        const selectedValues = Array.from(checkedBoxes).map(checkbox => checkbox.value);
