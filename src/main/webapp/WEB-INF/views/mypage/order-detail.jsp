     ...
     <div class="card d-flex flex-fill flex-column gap-3 overflow-auto">
         ...
         <div class="card progress-box d-flex gap-3">
                ...
                <div class="progress-stacked">
                    <div style="width: 20%;">
                        <div class="progress-step ${not empty request and (request.status == '의뢰 확인중' || request.status == '견적중' || request.status == '결제 진행' || request.status == '작업중' || request.status == '작업 완료') ? 'active' : ''}" 
                             data-step="0" data-status="${not empty request ? request.status : ''}">
                            <div class="prog-cont">
                    ...
                    <div style="width: 20%;">
                        <div class="progress-step ${not empty request and (request.status == '견적중' || request.status == '결제 진행' || request.status == '작업중' || request.status == '작업 완료') ? 'active' : ''}" 
                             data-step="1" data-status="${not empty request ? request.status : ''}">
                            <div class="prog-cont">
                                <span class="step-number"></span>
                            </div>
                        </div>
                    ...
                    <div style="width: 20%;">
                        <div class="progress-step ${not empty request and (request.status == '결제 진행' || request.status == '작업중' || request.status == '작업 완료') ? 'active' : ''}" 
                             data-step="2" data-status="${not empty request ? request.status : ''}">
                            <div class="prog-cont">
                                <span class="step-number"></span>
                            </div>
                        </div>
                    ...
                    <div style="width: 20%;">
                        <div class="progress-step ${not empty request and (request.status == '작업중' || request.status == '작업 완료') ? 'active' : ''}" 
                             data-step="3" data-status="${not empty request ? request.status : ''}">
                            <div class="prog-cont">
                                <span class="step-number"></span>
                            </div>
                        </div>
                    ...
                    <div style="width: 20%;">
                        <div class="progress-step ${not empty request and request.status == '작업 완료' ? 'active' : ''}" 
                             data-step="4" data-status="${not empty request ? request.status : ''}">
                            <div class="prog-cont">
                                <span class="step-number"></span>
                            </div>
                        </div>
             ...
             <div class="prog-pane bg-body-tertiary p-3 rounded-2" id="step1-panel" 
                  <c:if test="${not empty request and request.status == '의뢰 확인중'}">style="display: block;"</c:if>
                  <c:if test="${empty request or request.status != '의뢰 확인중'}">style="display: none;"</c:if>>
                     ...
                     <div class="form-group flex-column align-items-baseline gap-1">
                         ...
                         <c:if test="${not empty request and not empty request.filePath}">
                             <div class="text-success">
                                 <a href="/mypage/download-file/${request.id}" class="text-decoration-none">
                                     <i class="bi bi-download"></i> ${request.filePath}
                                 </a>
                             </div>
                         </c:if>
             ...
             <div class="prog-pane bg-body-tertiary p-3 rounded-2" id="step2-panel" 
                  <c:if test="${not empty request and request.status == '견적중'}">style="display: block;"</c:if>
                  <c:if test="${empty request or request.status != '견적중'}">style="display: none;"</c:if>>
             ...
             <div class="prog-pane bg-body-tertiary p-3 rounded-2" id="step3-panel" 
                  <c:if test="${not empty request and request.status == '결제 진행'}">style="display: block;"</c:if>
                  <c:if test="${empty request or request.status != '결제 진행'}">style="display: none;"</c:if>>
                 ...
                 <div class="bg-white p-3">
                     ...
                     <div class="d-flex justify-content-end gap-3 pt-1">총 결제금액 <span class="text-danger fw-bold"><fmt:formatNumber value="${not empty request ? request.paymentAmount : 0}" pattern="#,###" />원</span></div>
                 </div>
             </div>
             <div class="prog-pane bg-body-tertiary p-3 rounded-2" id="step4-panel" 
                  <c:if test="${not empty request and request.status == '작업중'}">style="display: block;"</c:if>
                  <c:if test="${empty request or request.status != '작업중'}">style="display: none;"</c:if>>
             ...
             <div class="prog-pane bg-body-tertiary p-3 rounded-2" id="step5-panel" 
                  <c:if test="${not empty request and request.status == '작업 완료'}">style="display: block;"</c:if>
                  <c:if test="${empty request or request.status != '작업 완료'}">style="display: none;"</c:if>>
                 ...
                 <div class="d-none d-lg-block bg-white p-3">
                     <c:choose>
                         ...
                         <c:otherwise>
                          ...
                          <tbody class="">
                              <c:choose>
                                  <c:when test="${not empty request and request.status == '작업 완료' and not empty request.adminFilePath}">
                                      <!-- admin_file_path에 저장된 파일들을 각각 행으로 표시 -->
                                      ...
                                      <c:forEach var="adminFile" items="${adminFiles}" varStatus="status">
                                          ...
                                          <td>
                                                      <button type="button" class="btn" onclick="downloadAdminFile('${fn:trim(adminFile)}')">
...
<script>
   ...
   // 현재 상태에 따른 허용된 단계 인덱스 반환
   function getAllowedStepIndex(status) {
       switch(status) {
           case '의뢰 확인중': return 0;
           case '견적중': return 1;
           case '결제 진행': return 2;
           case '작업중': return 3;
           case '작업 완료': return 4;
   ...
   const allowedStepIndex = getAllowedStepIndex(currentStatus);
   ...
   function initialize() {
       ...
       if (progressSteps.length > 0) {
           // 현재 상태에 해당하는 단계까지 활성화
           for (let i = 0; i <= allowedStepIndex; i++) {
               progressSteps[i].classList.add('active');
           }
           
           // 현재 상태에 해당하는 패널만 표시
           updatePanes(allowedStepIndex);
           updateActiveStepStyles();
       }
   }
   ...
   // .progress-step 클릭 시 이벤트 처리
   progressSteps.forEach((step, index) => {
       step.addEventListener('click', function() {
           handleStepClick(index);
       });
       
       // 클릭 불가능한 단계에 시각적 표시 추가
